...
//const BASE_URL = "https://www.sislo.com.br/Sislo_Enterprise/public/";//produção
...
function showErrors(error_list) {
    ...
    $.each(error_list, function (id, message) {
        $(id).parents().addClass("has-error");
        $(id).parents().siblings(".help-block").html(message);
    });
}
...
function buttonBack(route = "") {
    return '<br><div class="row"><a class="btn btn-primary" href="' + BASE_URL + route + '">Voltar</a><br><br></div>';
}

function success_note(note = "") {
    return '<h1 class="text text-success">' + note + ' Salvo com sucesso!!</h1>';
}

function error_note(note = "") {
    return '<h1 class="text text-error">' + note + ' Erro!!</h1>';
}

function converter_moeda(valor = 0) {
    var exclui_pontos = valor.replaceAll(".", "");
    var exclui_virgula = exclui_pontos.replaceAll(",", ".");
    return exclui_virgula;
}
...
function uploadImg(input_file, img, input_path) {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            clearErrors();
            input_path.siblings(".help-block").html(loadingImg("Carregando..."));
        },
