$(document).ready(function () {
    ...
    $("#sislo_comissao_jogos_bolao_loterias").validate({
        ...
        submitHandler: function (form) {
            $.ajax({
                type: "POST",
                url: BASE_URL + "ajax_save_formbolao",
                dataType: "html",
                data: $(form).serialize(),
                beforeSend: function () {
                    clearErrors();
                ...
                success: function (response) {
                    clearErrors();
                    var conteudo = response === '1' ? success_note('Comissão de jogos BOLÃO') + buttonBack('sislo_comissao_jogosboloes') : error_note('Comissão de jogos BOLÃO') + buttonBack('sislo_comissao_jogosboloes');
