$(document).ready(function () {
    ...
    $("#sislo_comissao_jogos_ibc_loterias").validate({
        messages: {
            referencia: {
        ...
        submitHandler: function (form) {
            $.ajax({
                type: "POST",
                url: BASE_URL + "sislo_comissao_jogos_ibc_loterias_form",
                dataType: "html",
                data: $(form).serialize(),
                beforeSend: function () {
                    clearErrors();
                ...
                success: function (response) {
                    clearErrors();
                    var conteudo = response === '1' ? success_note('Comissão de jogos') + buttonBack('sislo_comissao_ibc') : error_note('Comissão de jogos') + buttonBack('sislo_comissao_ibc');
                    $('#sislo_comissao_jogos_ibc_loterias')[0].reset();
                    $("#conteudo").empty();
                    if (response === '1') {
