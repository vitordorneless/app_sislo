<div class="card">
    <div class="card-header">
        <h3 class="card-title">Ajustar Horas Empregado</h3>
    </div>
    <div class="card-body">
        <br>
        <div class="row">
            <form class="form-inline" id="sislo_horas_list" method="POST">
                <?php
                $ano = $anos = date('Y');
                $mes = date('m');
                ?>
                <div class="form-group">
                    <label class="text text-sm">Funcionário</label>
                    <select id="id_usuario" name="id_usuario" class="form-control">
                        <option value="0">Selecione...</option>
                        <?php
                        foreach ($funcionario as $value) {
                            echo '<option value="' . $value->idsislo_funcionarios . '">' . $value->nome . '</option>';
                        }
                        ?>
                    </select>
                </div>
    ...
    <div class="card-footer" id="conteudo">
        ...
        <div class="row">
            <table id="table_sislo_horas_list" class="table table-striped table-bordered table-responsive text text-sm text-center">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Nome</th>
