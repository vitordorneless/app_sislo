...
class Sislo_ComissaoBolaoOnline extends BaseController {
    ...
    public function redireciona_bolaoonline_edit() {
        if ($this->session->get('user_id')) {
            $sislo_usuarios_model = new \App\Models\Sislo_UsuariosModel;
            $sislo_model = new \App\Models\Sislo_ComissaoBolaoOnlineModel;
            $dadosuser = $sislo_usuarios_model->find($this->session->get('user_id'));
            $sislo_jogos = new \App\Models\SisloJogosCefModel;
            $jogos = $sislo_jogos->where('status', 1)->orderBy('nome', 'asc')->findAll();
            ...
            $data = array(
                "scripts" => array(
                    "sislo_comissao_bolaoonline_crud_edit.js",
                    "sweetalert2.all.min.js",
                    "jquery.validate.js",
                    "jquery.mask.min.js",
                    "jquery.maskMoney.min.js",
            ...
            echo view('template/menu');
            echo view('template/content');
            echo view('sislo_comissao_bolaoonline_crud_edit', $data);
            echo view('template/footer', $data);
            echo view('template/scripts', $data);
        } else {
            echo view('login');
