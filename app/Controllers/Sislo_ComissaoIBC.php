...
class Sislo_ComissaoIBC extends BaseController {
    ...
    public function index() {
        if ($this->session->get('user_id')) {
            ...
            echo view('sislo_comissao_ibc', $data);
    ...
    public function redireciona_comissao_jogosibc() {
        if ($this->session->get('user_id')) {
            ...
            if ($this->request->getGet('id') == '0') {
                ...
                echo view('template/menu');
                echo view('template/content');
                echo view('sislo_comissao_ibc_crud', $data);
                echo view('template/footer', $data);
                echo view('template/scripts', $data);
            } else {
                $incluir = 2;
    ...
    public function carrega_comissoes_ibc() {
        ...
        $query = $builder->select("scs.idsislo_comissao_ibc as idsislo_comissao_ibc, "
    ...
    public function ajax_list_comissao_ibc() {//fazer o redireciona FAZER o formulario para incluir, form para editar
        if ($this->request->isAJAX()) {
            $sislo_comissao = $this->carrega_comissoes_ibc()->getResult();
            $data = array();
            ...
            foreach ($sislo_comissao as $value) {
                ...
                $row[] = '<a class="btn btn-primary" href="' . base_url('redireciona_comissao_jogosibc/?id=' . $value->idsislo_comissao_ibc) . '">Editar</a>';
                ++$tt;
                ++$tb;
                $data[] = $row;
            }
    ...
    public function ajax_save_form() {
        if ($this->request->isAJAX()) {
            $cob = new \App\Models\Sislo_ComissaoIBCModel;
            $datas = array();
            ...
            if ($this->request->getPost('incluir') == '1') {
                ...
                $sislo_notificacao_model->set('notificacao', 'Comissão de IBC Inserida');
