...
class Sislo_StatusVagas extends BaseController {
    ...
    public function index() {
        if ($this->session->get('user_id')) {
            ...
            echo view('sislo_status_vagas', $data);
    ...
    public function ajax_list_vagas() {
        if ($this->request->isAJAX()) {
            $sislo_status_vagas_model = new \App\Models\Sislo_StatusVagaModel;
            $sislo = $sislo_status_vagas_model->where('status', 1)->orderBy('nome_status', 'asc')->findAll();
            $data = array();
            ...
            foreach ($sislo as $value) {
                ...
                $row[] = '<a class="btn btn-primary" href="' . base_url('redireciona_vagas/?id=' . $value->id_sislo_status_vaga) . '">Editar</a>';
                ++$tt;
                ++$tb;
                $data[] = $row;
            }
    ...
    public function redireciona_vagas() {
        if ($this->session->get('user_id')) {
            $sislo_usuarios_model = new \App\Models\Sislo_UsuariosModel;
            $sislo_model = new \App\Models\Sislo_StatusVagaModel;
            ...
            echo view('template/menu');
            echo view('template/content');
            echo view('sislo_status_vagas_crud', $data);
            echo view('template/footer', $data);
            echo view('template/scripts', $data);
        } else {
            echo view('login');
    ...
    public function ajax_save_form() {
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($this->request->getPost('incluir') == '1') {
                ...
                $sislo_model->where('id_sislo_status_vaga', $this->request->getPost('id_sislo_status_vaga'));
