...
class Sislo_Timemania extends BaseController {
    ...
    public function index() {
        if ($this->session->get('user_id')) {
            ...
            echo view('sislo_timemania', $data);
    ...
    public function carrega_timemania() {
        $db = \Config\Database::connect();
        $builder = $db->table('sislo_timemania AS timemania');
        $query = $builder->select("timemania.id_sislo_timemania_time_coracao,timemania.idsislo_timemania,timemania.concurso,timemania.data_concurso,timemania.dez_01,timemania.dez_02,timemania.dez_03,timemania.dez_04,
                                timemania.dez_05,timemania.dez_06,timemania.dez_07,time_coracao.time_coracao,timemania.saiu_ganhador,timemania.premio_atual,
                                timemania.premio_acumulado,timemania.arrecadacao_total")
                ->join("sislo_timemania_time_coracao AS time_coracao", "timemania.id_sislo_timemania_time_coracao = time_coracao.idsislo_timemania_time_coracao", "inner")
                ->orderBy('timemania.concurso', 'DESC')
                ->get();
        return $query;
    }

    public function ajax_list_timemania() {
        if ($this->request->isAJAX()) {                        
            $sislo = $this->carrega_timemania()->getResult();
            $data = array();
            ...
            foreach ($sislo as $value) {                
                ...
                $row[] = '<a class="btn btn-primary" href="' . base_url('redireciona_timemania/?id=' . $value->idsislo_timemania) . '">Editar</a>';
                ++$tt;
                ++$tb;
                $data[] = $row;
            }
    ...
    public function redireciona_timemania() {
        if ($this->session->get('user_id')) {
            $sislo_usuarios_model = new \App\Models\Sislo_UsuariosModel;
            $sislo_model = new \App\Models\Sislo_TimemaniaModeL;            
            $sislo_jogos = new \App\Models\Sislo_TimemaniaTimeCoracaoModel;            
            $jogos = $sislo_jogos->orderBy('time_coracao', 'asc')->findAll();
            ...
            if ($this->request->getGet('id') == '0') {
                ...
                $dados['dez_06'] = '';                
                $dados['dez_07'] = '';                
                $dados['id_sislo_timemania_time_coracao'] = '';                
                ...
                $dados['idsislo_timemania'] = $dados_loterica->idsislo_timemania;
                ...
                $dados['id_sislo_timemania_time_coracao'] = $dados_loterica->id_sislo_timemania_time_coracao;
            ...
            $data = array(
                ...
                "incluir" => $incluir,    
                "jogos" => $jogos,
                "idsislo_timemania" => $dados['idsislo_timemania'],
                ...
                "dez_06" => $dados['dez_06'],
                "dez_07" => $dados['dez_07'],
                "id_sislo_timemania_time_coracao" => $dados['id_sislo_timemania_time_coracao'],
            ...
            echo view('template/menu');
            echo view('template/content');
            echo view('sislo_timemania_crud', $data);
            echo view('template/footer', $data);
            echo view('template/scripts', $data);
        } else {
            echo view('login');
    ...
    public function ajax_save_form() {
        ...
        if ($this->request->isAJAX()) {
            ...
            $sislo_model->set('id_sislo_timemania_time_coracao', $this->request->getPost('id_sislo_timemania_time_coracao'));
            ...
            if ($this->request->getPost('incluir') == '1') {
                ...
                $sislo_model->where('idsislo_timemania', $this->request->getPost('idsislo_timemania'));
