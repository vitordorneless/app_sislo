...
class Sislo_ContaCorrente extends BaseController {
    ...
    public function index() {
        if ($this->session->get('user_id')) {
            ...
            $data = array(
                "scripts" => array(
                    "sislo_contacorrente.js", "jquery.validate.js",
            ...
            echo view('sislo_contacorrente', $data);
    ...
    public function ajax_list_contacorrente() {
        if ($this->request->isAJAX()) {
            $sislo_contacorrente = new \App\Models\Sislo_ContaCorrenteModel;
            $referencia = $this->request->getPost('mes') . '/' . $this->request->getPost('ano');
            $sislo_contas = $sislo_contacorrente->where('referencia', $referencia)->where('status', 1)->orderBy('data_transacao', 'asc')->findAll();
            $data = array();
