...
class Sislo_TipoPEC extends BaseController {
    ...
    public function ajax_list() {
        if ($this->request->isAJAX()) {
            $sislo_tipo_pec_model = new \App\Models\Sislo_TipoPECModel;
            $sislo = $sislo_tipo_pec_model->where('status', 1)->orderBy('tipo', 'asc')->findAll();
            $data = array();
            ...
            foreach ($sislo as $value) {
                ...
                $row[] = '<a class="btn btn-primary" href="' . base_url('redireciona/?id=' . $value->idsislo_tipo_pec) . '">Editar</a>';
                ++$tt;
                ++$tb;
                $data[] = $row;
            }
    ...
    public function redireciona() {
        if ($this->session->get('user_id')) {
            ...
            $sislo_model = new \App\Models\Sislo_TipoPECModel;
            ...
            if ($this->request->getGet('id') == '0') {
                $incluir = 1;
                $dados['idsislo_tipo_pec'] = '';
                $dados['tipo'] = '';
                $dados['status'] = '';
            } else {
                $incluir = 2;
                $dados_loterica = $sislo_model->find($this->request->getGet('id'));
                $dados['idsislo_tipo_pec'] = $dados_loterica->idsislo_tipo_pec;
            ...
            $data = array(
                ...
                "incluir" => $incluir,
                "tipo" => $dados['tipo'],
                "idsislo_tipo_pec" => $dados['idsislo_tipo_pec'],
                "status" => $dados['status']
            );
            echo view('template/header', $data);
            echo view('template/menu');
            echo view('template/content');
            echo view('sislo_tipo_pec_crud', $data);
            echo view('template/footer', $data);
            echo view('template/scripts', $data);
        } else {
            echo view('login');
    ...
    public function ajax_save_form() {
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($this->request->getPost('incluir') == '1') {
                ...
                $sislo_model->where('idsislo_tipo_pec', $this->request->getPost('idsislo_tipo_pec'));
